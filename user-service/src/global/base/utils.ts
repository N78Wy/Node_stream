...
import { createCipher, createDecipher, createHash, HexBase64Latin1Encoding, randomBytes } from "crypto";
import { exists, readdirSync, readFile, writeFile } from "fs";
...
export function createRequestId(): string {
  return Date.now().toString(32) + Math.random().toString(32).slice(2) + Math.random().toString(32).slice(2);
}
...
/** 获取日期时间字符串（2018-01-01 12:30:59）/ （2018/01/01 12:30:59*/
export function getDateTimeString(pad = " ", time = new Date(), seq = "-") {
  return `${getDateString(seq, time)}${pad}${getTimeString(":", time)}`;
}
...
export function mergeUrl(dist: string, query: any, hash: string) {
  ...
  distUrl.search = stringify(distUrl.query);
  distUrl.hash = hash;
 ...
 * @export
 * @param {string} str
 * @param {string} [outputEncoding="hex"]
 */
export function sha256(str: string, outputEncoding: HexBase64Latin1Encoding = "hex") {
  return createHash("sha256").update(str).digest(outputEncoding);
}
...
export function getShortName(name: string) {
  return (
    // 去除私有域,如 @types/test 变为 test
    (name.split("/")[1] || name)
      // 按中划线,下划线或空格分隔
      .split(/[-_ ]/)
      // 忽略node开头和server结尾
      .filter((s, i, arr) => !((i === 0 && s === "node") || (i === arr.length - 1 && s === "server")))
      // 取前几个单词首字母加最后一个单词
      .map((s, i, arr) => (i === arr.length - 1 ? s : s[0]))
      .join("")
  );
}
...
export function requireDir(
  dir: string,
  {
    ...
    excludeFile?: string[];
    onRequired?: (params: { filepath: string; filename: string }) => void;
  }
) {
  const list = readdirSync(dir, "utf8");
  for (const filename of list) {
    if (![".ts", ".js"].some((ext) => filename.endsWith(ext)) || [".d.ts"].some((ext) => filename.endsWith(ext))) {
      continue;
    }

    const filenameNExt = path.basename(filename, path.extname(filename));
    if (excludeFile && excludeFile.some((filename) => filenameNExt === filename)) {
      continue;
    }
    ...
    try {
      ...
      console.log(`模块[${filepath}]加载失败`, err);
      process.exit(1);
    }
    onRequired?.({
      filepath,
...
export function parseDependency(dependency: string) {
  const chunk = dependency.split("/");
  const scope = chunk.length === 1 ? "" : chunk[0];
  const [name, version] = chunk[chunk.length === 1 ? 0 : 1].split("@");
...
export function assignDefined<T, U, V>(target: T, source1: U, source2: V): T & U & V;
export function assignDefined<T, U, V, W>(target: T, source1: U, source2: V, source3: W): T & U & V & W;
export function assignDefined(target: object, ...sources: any[]): any;
export function assignDefined(target: object, ...sources: any[]) {
